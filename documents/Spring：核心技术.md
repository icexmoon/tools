...
`request` 、 `session` 、 `application` 和 `websocket` 作用域仅在您使用Web感知Spring `ApplicationContext` 实现（如 `XmlWebApplicationContext` ）时可用。如果您将这些作用域与常规Spring 

IoC容器（如 `ClassPathXmlApplicationContext` ）一起使用，则会抛出一个 `IllegalStateException` ，抱怨未知的bean作用域。
